...
void activite::exporterpdf_activite(QTextBrowser *text)
{
    ...
    QString tt;
    QSqlQuery qry;
    qry.exec("select* from activite");
    while(qry.next())
    {
        tt="Id: "+qry.value(0).toString()+"\n"+"Num_App: "+qry.value(1).toString()+"\n"+"Num_Bloc: "+qry.value(2).toString()+"\n"+"Num_Res: "+qry.value(3).toString()+"Prix_Frais: "+qry.value(4).toString()+" TND";
    }
                text->setText(tt);
